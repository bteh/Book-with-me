...
exports.authMiddleware = function(req,res,next){
	...
	if(token){
		...
		return res.status(422).send({errors: [{title: 'Not authorized', detail: 'You need to login for access'}]});
	}
}
...
function notAuthorized(res){
		return res.status(401).send({errors: [{title: 'Not authorized', detail: 'You need to login for access'}]});
}